...
这个场景不是“忘记提交”，而是：事务在等待锁期间一直没结束，看起来就像“长事务”，并且会把自己已持有的锁继续占着，导致更多请求排队。

```bash
# Tx-B：先把某个订单行锁住一段时间
curl "http://localhost:18888/deadlock/lock-wait/hold-order?orderNo=ORD20241001001&holdMs=20000"

# Tx-A：先锁账户，再去锁同一订单行（会等待Tx-B；同时自己占着账户锁）
curl "http://localhost:18888/deadlock/lock-wait/chain-a?accountNo=ACC001&orderNo=ORD20241001001"

# Tx-C：再来触碰同一账户（会等待Tx-A）
curl "http://localhost:18888/deadlock/lock-wait/touch-account?accountNo=ACC001"
```

### 4.Y 手动事务忘记commit/close -> 持锁+连接泄漏（新增接口）

这个场景更贴近“手动开事务，最后忘记commit/close”：连接一直不归还连接池 + 事务一直不结束，InnoDB行锁会一直持有。

```bash
# 1) 创建一个“泄漏事务”：锁住 account(ACC001) 并 UPDATE，但不commit/close
curl "http://localhost:18888/deadlock/tx-leak/leak-account-lock?accountNo=ACC001&delta=0.01"

# 2) 另一个请求去更新同一行（会卡住/等待锁）
curl "http://localhost:18888/deadlock/lock-wait/touch-account?accountNo=ACC001"

# 3) 如果想更快看到“雪崩”（连接池耗尽），多泄漏几个连接（不持锁，只占连接）
curl "http://localhost:18888/deadlock/tx-leak/leak-conn"

# 4) 恢复：一键回滚并关闭所有泄漏连接
curl "http://localhost:18888/deadlock/tx-leak/close-all"
```
